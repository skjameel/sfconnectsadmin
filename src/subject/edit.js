...
import {
    getSubjectsDetails, updateSubjects
} from './actions';
import {
    StandardsList
...
class Edit extends React.Component {
    ...
    componentWillMount() {
        ...
        const queryparams = {
            customizeQuery: `select * from subject where Id=${this.props.match.params.id}`,
        };
        this.props.getSubjectsDetails(this.props.match.params.id, queryparams).then(res => {
            ...
            this.props.form.setFieldsValue({ subject_name: data.subject_name })
    ...
    handleSubmit = e => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                const { subject_name, Description, StandardId, SchoolId, Isactive, createdBy } = values;
                ...
                this.props.updateSubjects(this.props.match.params.id, queryparams).then(res => {
                    const { value: { success, data } } = res;
    ...
    render() {
        ...
        console.log("11111>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", this.props.subject)
        return (
            <div>
                <div style={{ margin: '2%' }}>
                    <h2> Edit Page</h2>
