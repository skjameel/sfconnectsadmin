...
class List extends React.Component {
  ...
  onClick = (dataSource) => {
    ...
    this.props.history.push(`city-view/${dataSource.Id}`)
  }
  onClickEdit = (dataSource) => {
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>.", dataSource, this.props)
    this.props.history.push(`city-edit/${dataSource.Id}`)
  }
  onClickDelete = (dataSource) => {
    console.log("DELETEEEEEEEEEEE");
    const queryparams = {
      customizeQuery: `UPDATE city SET Isactive='InActive' WHERE Id=${dataSource.Id} `,
    };
    console.log("idddddddddddddd", dataSource.Id)
    this.props.deleteCity(this.props.match.params.Id, queryparams).then(res => {
  ...
  render() {
    console.log('>>>>>>>>>>>>>>>>>mmmmmmmmmm', this.props.city);
      ...
      <div className="col-lg-auto">
        <div className="col-md-auto" style={{ marginTop: '1%', marginBottom: '5%' }}>
          ...
          <div class="col-sm-6 pull-right text-right">
            <a class="btn btn-primary mb-3" style={{color:'White'}} onClick={() => this.props.history.push('/add-city')} role="button"><i class="ti-plus" data-toggle="dropdown"></i> Add Cities</a>
          </div>
        </div>
        <div className="col-md-auto">
          <Table columns={this.state.columns} dataSource={this.props.city.CityList} bordered />
        </div>
      </div>
    );
  }
