...
class List extends React.Component {
  ...
  componentWillMount() {
    ...
    const queryparams = {
      customizeQuery: `select * from adminlistview where Isactive='Active' order by Id desc;`,
    };
    this.props.AdminList(queryparams);
  ...
  onClick = (dataSource) => {
    ...
    this.props.history.push(`admin-view/${dataSource.Id}`)
  }
  onClickEdit = (dataSource) => {
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>.", dataSource, this.props)
    this.props.history.push(`admin-edit/${dataSource.Id}`)
  }
  onClickDelete = (dataSource) => {
    console.log("DELETEEEEEEEEEEE");
    const queryparams = {
      customizeQuery: `UPDATE admin SET Isactive='InActive' WHERE Id=${dataSource.Id} `,
    };
    console.log("idddddddddddddd", dataSource.Id)
    this.props.deleteAdmin(this.props.match.params.Id, queryparams).then(res => {
  ...
  render() {
    console.log('>>>>>>>>>>>>>>>>>mmmmmmmmmm', this.props.admin);
      ...
      <div className="col-lg-auto">
        <div className="col-md-auto" style={{ marginTop: '1%', marginBottom: '5%' }}>
          ...
          <div class="col-sm-6 pull-right text-right">
            <a class="btn btn-primary mb-3" style={{color:'White'}} onClick={() => this.props.history.push('/add-admin')} role="button"><i class="ti-plus" data-toggle="dropdown"></i> Add Admins</a>
          </div>
        </div>
        <div className="col-md-auto">
          <Table columns={this.state.columns} dataSource={this.props.admin.AdminList} bordered />
        </div>
      </div>
    );
  }
